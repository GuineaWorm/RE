...
if killTame:
    renameTamedAnimalsTo = 'KillMe'
    killList = []
else:
    renameTamedAnimalsTo = 'asd'
...
if Player.GetRealSkillValue('Animal Taming') < 30:
...
animals = {
    ...
    'gaman': Animal( 'gaman', 0x00F8, 0x0000, 68, 90, None),
    'slithStone': None,
    ...
    #Custom
    'tigerSabreToothed': Animal( 'saber-toothed tiger', 0x0588, 0x0000, 100, 120, None ),
    'reptalon': Animal( 'a reptalon', 0x0114, 0x0000, 100, 120, None ),
    'nightmare1': Animal( 'a nightmare', 0x0074, 0x0000, 100, 120, None ),
    'nightmare2': Animal( 'a nightmare', 0x00B1, 0x0000, 100, 120, None ), 
    'nightmare3': Animal( 'a nightmare', 0x00B2, 0x0000, 100, 120, None ), 
    'nightmare4': Animal( 'a nightmare', 0x00B3, 0x0000, 100, 120, None ),     
    'unicorn': Animal( 'a unicorn', 0x007A, 0x0000, 100, 120, None ), 
    'triceratops': Animal( 'a triceratops', 0x0587, 0x0000, 100, 120, None),
    ...
    'phoenix': Animal( 'phoenix', 0x0340, 0x0000, 110, 120, None ),
...
def TrainAnimalTaming():
    ...
    while not Player.IsGhost:
        ...
        if not tameOngoing and not Timer.Check( 'skillTimer' ):
            ...
            #Player.HeadMessage(33,"33")
            Misc.Pause(100)

            # Check if Animal Taming was successfully triggered
            if Journal.Search( 'You seem to anger the beast!' ):
                Journal.Clear()
                while not Journal.Search('*You start to tame the creature.*'):
                    Player.UseSkill( 'Animal Taming' )
                    Target.WaitForTarget( 2000, False )
                    Target.TargetExecute( animalBeingTamed )
                #Player.HeadMessage(33,"33")
                #tameOngoing = False
                Journal.Clear()
            elif Journal.Search( 'Tame which animal?' ):
                Player.HeadMessage(33,"22")
                timesTried += 1
        ...
        if tameOngoing:
            ...
            if ( Journal.SearchByName( 'It seems to accept you as master.', animalBeingTamed.Name ) or
                ...
                tameHandled = True
                    ...
                    Journal.Search( 'That creature cannot be tamed.') or
